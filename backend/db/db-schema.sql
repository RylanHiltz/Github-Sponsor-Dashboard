...
CREATE INDEX if not EXISTS idx_sponsorship_sponsor_id ON public.sponsorship(sponsor_id);
CREATE INDEX if not EXISTS idx_sponsorship_sponsored_id ON public.sponsorship(sponsored_id);


CREATE TABLE public.sponsorship_graph_layout (
  user_id bigint not null,
  x real not null,
  y real not null,
  z real not null,
  ...
  constraint sponsorship_graph_layout_user_id_fkey foreign KEY (user_id) references users (id) on delete CASCADE
) TABLESPACE pg_default;
